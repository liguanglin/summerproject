...
{
	//printf("%d %d\n", p1, p1);
	return sqr(lumR[p1] - lumR[p2]) + sqr(lumG[p1] - lumG[p2]) + sqr(lumB[p1] - lumB[p2]);
}

int smoothFn(int p1, int p2, int l1, int l2)
{
	double t = (u2[p1]+u2[p2])/ (dis(p1,p2) + 1)*std::abs(l1 - l2);
	t *= 30;
	//if (t > 10) printf("%lf\n", t);
	return (int)t;
}
	...
	for (int i = 10; i <= 10; i++) {
		for (int x = 0; x < height; x++) {
			for (int y = 0; y < width; y++) {
				ux[x][y] = 0;
				lumR[x*width + y] = frames[i].color_img.at<cv::Vec3b>(x, y)[0];
				lumG[x*width + y] = frames[i].color_img.at<cv::Vec3b>(x, y)[1];
				lumB[x*width + y] = frames[i].color_img.at<cv::Vec3b>(x, y)[2];
				for (int d = 0; d < k; d++) {
					L_init[x][y][d] = 0;
				}
			}
		...
		for (int d = 1; d <= k; d++) {
			for (int j = 0; j < 30; j++) {
				...
				uchar* pic1 = frame->color_img.data;
				uchar* pic2 = framet->color_img.data;
				//cout << framet->cam->R << "???\n";
				...
				for (int x = 0; x < height; x++) {
					for (int y = 0; y <width; y++) {
						...
						if (tx >= 0 && tx < height&&ty >= 0 && ty < width) {
							int src1 = x * width + y, src2 = tx * width + ty;
							//trans.at<uchar>(tx, ty) = frame->gray_img.at<uchar>(x, y);
							/*int tmpR = abs(framet->color_img.at<cv::Vec3b>(tx, ty)[0] - frame->color_img.at<cv::Vec3b>(x, y)[0]);
							int tmpG = abs(framet->color_img.at<cv::Vec3b>(tx, ty)[1] - frame->color_img.at<cv::Vec3b>(x, y)[1]);
							int tmpB = abs(framet->color_img.at<cv::Vec3b>(tx, ty)[2] - frame->color_img.at<cv::Vec3b>(x, y)[2]);*/
							int tmpR = abs(pic2[src2 * 3] - pic1[src1 * 3]);
							int tmpG = abs(pic2[src2 * 3 + 1] - pic1[src1 * 3 + 1]);
							int tmpB = abs(pic2[src2 * 3 + 2] - pic1[src1 * 3 + 2]);
							tmp = 1 / (1 + tmpR * tmpR + tmpG * tmpG + tmpB * tmpB);
						}
						L_init[x][y][d - 1] += tmp;
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				...
				if (i > 0) sumg += 1, sum += 1.0 / (dis(i*width + j, (i - 1)*width + j) + 1);
				if (j > 0) sumg += 1, sum += 1.0 / (dis(i*width + j, i*width + j - 1) + 1);
				if (i < height - 1) sumg += 1, sum += 1.0 / (dis(i*width + j, (i + 1)*width + j) + 1);
				if (j < width - 1) sumg += 1, sum += 1.0 / (dis(i*width + j, i*width + j + 1) + 1);
				if (sum < 0) {
					printf("%lf\n", sum);
				}
		...
		for (int x = 0; x < height; x++) {
			for (int y = 0; y < width; y++) {
				...
				//}
				//if (ans != 0) printf("%d %d\n", x, y);
				frame->dep_img.at<uchar>(x, y)= res[tot++]*200/k+55;
			}
		}
		char file_name[20];
		sprintf(file_name, "depth%d.bmp", i);
